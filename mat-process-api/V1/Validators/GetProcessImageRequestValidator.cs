...
namespace mat_process_api.V1.Validators
{
    public class GetProcessImageRequestValidator : AbstractValidator<GetProcessImageRequest>, IGetProcessImageRequestValidator
    {
        public GetProcessImageRequestValidator()
        {
            ...
            RuleFor(req => req.processRef).NotNull().WithMessage("Process reference must be provided.").NotEmpty().WithMessage("Process reference must not be empty.") //Added the NotNull,NotEmpty checks back to get more specific error messages
                .Must(ValidateGuid).WithMessage("You need to provide a valid process reference.");
            RuleFor(req => req.imageId).NotNull().WithMessage("Image Id must be provided.").NotEmpty().WithMessage("Image Id must not be empty.")
                .Must(ValidateGuid).WithMessage("You need to provide a valid Image Id.");
            RuleFor(req => req.processType).NotNull().WithMessage("Process Type must be provided.").NotEmpty().WithMessage("Process Type must not be empty.");
            RuleFor(req => req.fileExtension).NotNull().WithMessage("File Extension must be provided.").NotEmpty().WithMessage("File Extension must not be empty.");
        }
